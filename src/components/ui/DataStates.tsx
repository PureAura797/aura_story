...
export function EmptyState({ title = "Нет данных", description }: { title?: string; description?: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <Inbox className="w-8 h-8 text-neutral-600 mb-4" strokeWidth={1} />
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      {onRetry && (
        <button
          onClick={onRetry}
          className="flex items-center gap-1.5 mt-4 text-xs text-neutral-400 hover:text-white transition-colors uppercase tracking-wider cursor-pointer"
        >
...
export function SkeletonFAQ() {
  return (
    <div className="space-y-3 w-full">
      {[...Array(4)].map((_, i) => (
        <div key={i} className="border border-white/[0.06] p-5">
...
export function SkeletonCards({ count = 3 }: { count?: number }) {
  return (
    <div className="space-y-3 w-full">
      {[...Array(count)].map((_, i) => (
        <div key={i} className="border border-white/[0.06] p-6 space-y-3">
