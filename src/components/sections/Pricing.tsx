...
export default function Pricing() {
  ...
  return (
    <section ref={containerRef} className="w-full flex flex-col items-start z-10 relative">
      ...
      <p className="text-sm text-[var(--text-secondary)] font-light mb-12 max-w-md">{t("pricing.desc")}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full max-w-5xl">
        {plans.map((plan) => (
          <div key={plan.id} className="pricing-card card-lift opacity-0 translate-y-10 border border-[var(--border)] bg-[var(--glass-card)] backdrop-blur-sm p-6 group hover:border-[rgba(94,234,212,0.2)] transition-all duration-500 flex flex-col">
            ...
            <span className="text-[11px] uppercase tracking-[0.15em] text-[var(--text-muted)] border border-[var(--border)] px-3 py-1 w-fit mb-3">{plan.area}</span>
            <p className="text-[var(--text-secondary)] text-xs leading-relaxed mb-4 flex-1">{plan.description}</p>
            <p className="text-[11px] uppercase tracking-[0.15em] font-medium mb-4" style={{ color: "var(--sand-dim)" }}>{plan.features}</p>
            <div className="mt-auto pt-3 border-t border-[var(--border)]">
              <span className="text-[11px] text-[var(--text-secondary)] uppercase tracking-widest block mb-1">{t("pricing.from")}</span>
              <div className="text-2xl md:text-3xl font-bold tracking-tighter text-[var(--text-primary)]">{formatPrice(plan.price)}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="pricing-card opacity-0 translate-y-10 mt-4 border border-[var(--border)] bg-[var(--glass-card)] backdrop-blur-sm p-6 w-full max-w-5xl">
        <div className="flex flex-wrap gap-x-8 gap-y-2 text-[11px] uppercase tracking-[0.15em] text-[var(--text-secondary)] font-medium justify-center">
