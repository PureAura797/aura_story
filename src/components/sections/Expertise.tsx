...
export default function Expertise() {
  ...
  return (
    <section ref={containerRef} className="w-full flex flex-col items-start z-10 relative">
      ...
      <p className="text-sm text-[var(--text-secondary)] font-light mb-12 max-w-md">
        {t("expertise.desc")}
      </p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-3xl">
        {items.map((item, idx) => (
          <div key={idx} className="expertise-item card-lift accent-glow-hover opacity-0 translate-y-10 border border-[var(--border)] bg-[var(--glass-card)] backdrop-blur-sm p-8 group hover:border-[var(--sand)]/20 transition-all duration-500">
      ...
      <div className="expertise-item opacity-0 translate-y-10 mt-8 border border-[var(--border)] bg-[var(--glass-card)] backdrop-blur-sm w-full max-w-3xl overflow-hidden">
        <div className="flex flex-wrap gap-2 px-8 pt-8 pb-4">
          {steps.map((step, i) => (
            <span key={i} className="flex items-center gap-2">
              <span className="px-3 py-1.5 text-[11px] tracking-[0.1em] font-medium border border-[var(--border)] bg-[var(--glass-card)]"
                style={{ color: i === steps.length - 1 ? "var(--accent)" : "var(--sand-dim)" }}>
                {step}
              </span>
              {i < steps.length - 1 && <span className="text-[var(--text-secondary)] text-xs">→</span>}
            </span>
          ))}
        </div>
        
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 px-8 pb-8 pt-4 border-t border-[var(--border)]">
          <p className="text-[var(--text-secondary)] text-sm leading-relaxed max-w-sm">{t("expertise.cta_text")}</p>
