...
const LETTERS = ["а", "у", "р", "а", "ч", "и", "с", "т", "о", "т", "ы", ".", "р", "ф"];
const ACCENT_FROM = 11; // ".", "р", "ф" — accent colored
...
export default function Preloader() {
  ...
  useEffect(() => {
    ...
    const parts = [leafRef.current, leftRef.current, rightRef.current].filter(Boolean) as SVGPathElement[];
    const letters = lettersRef.current.filter(Boolean) as HTMLSpanElement[];

    document.body.style.overflow = "hidden";

    /* ─── Initial states ─── */
    // Emblem parts: invisible, heavy blur, scaled up
    gsap.set(leafRef.current, { opacity: 0, scale: 2.8, filter: "blur(24px)", transformOrigin: "center center" });
    gsap.set(leftRef.current, { opacity: 0, scale: 2.2, x: -10, filter: "blur(20px)", transformOrigin: "center center" });
    gsap.set(rightRef.current, { opacity: 0, scale: 2.2, x: 10, filter: "blur(20px)", transformOrigin: "center center" });

    // Letters: invisible, offset right with parallax
    letters.forEach((l, i) => {
      const offset = 30 + (LETTERS.length - i) * 8;
      gsap.set(l, { opacity: 0, x: offset, filter: "blur(6px)" });
    });
    ...
    // Phase 1: Leaf materialises from heavy blur
    ...
    // Phase 4: Letters slide in with parallax stagger
    tl.to(letters, {
    ...
    // Counter appears + counts
    tl.to(counterRef.current, { opacity: 1, duration: 0.2 }, 0.05);
    tl.to(counter, {
      value: 100,
      duration: 1.8,
      ease: "power2.inOut",
    ...
    // Phase 5: Hold — let it breathe
    tl.to({}, { duration: 0.35 });

    // Phase 6: Exit — evaporate (scale up + blur + fade)
    // Counter fades
    tl.to(counterRef.current, {
      opacity: 0, filter: "blur(8px)", scale: 1.1,
      duration: 0.6, ease: "power2.in",
    });

    // Entire logo wrap: scale up + blur + dissolve — evaporation
    tl.to(logoWrapRef.current, {
      scale: 1.15, filter: "blur(30px)", opacity: 0,
  ...
  return (
    ...
    >
      ...
      {/* Logo: emblem + wordmark "аурачистоты.рф" */}
      <div
        ref={logoWrapRef}
        className="relative z-20 flex items-center gap-4"
        style={{ willChange: "transform" }}
      >
        {/* SVG Emblem — 3 separate paths */}
        <svg
        ...
        >
          <g fill="white" fillRule="evenodd" style={{ overflow: "visible" }}>
            <path
              ref={leftRef}
              d={PATH_LEFT}
        ...
        {/* Wordmark: аурачистоты.рф — per-letter parallax animation */}
        <div className="flex gap-[1px]" style={{ fontFamily: "var(--font-unbounded), sans-serif" }}>
          {LETTERS.map((char, i) => (
            <span
              key={i}
              ref={setLetterRef(i)}
              className="inline-block text-[18px] md:text-[22px] font-light lowercase"
              style={{
                color: i >= ACCENT_FROM ? "var(--accent)" : "white",
      ...
      <span
        ref={counterRef}
        className="absolute bottom-10 right-12 z-30 text-[11px] tracking-[0.15em] text-neutral-500 tabular-nums"
        style={{ fontFamily: "var(--font-jost), sans-serif", fontWeight: 100, opacity: 0 }}
      >
        0
      </span>
    </div>
